				...
				<div class="tabbable">
					<div class="tab-content">
						<div class="tab-pane active" id="S">
							<!-- <div class="well"><label>拉新奖励：只有VIP会获取此奖励<br/>
									充值奖励：只有代理人会获取此奖励</label></div> -->
							<fieldset>
								<div class="well"><label>拉新奖励：当推荐人A推荐新会员B时<br/>
									VIP说明：1,获取油卡 2,购买年费<br/>
									1,当推荐人A推荐新会员B时<br/>
									2,当新会员B第一次申领油卡时,如果套餐为普通套餐,无奖励，即使之后再次申领VIP套餐，也无奖励。<br/>
									3,当新会员B第一次申领油卡时,如果套餐为VIP套餐，将按百分比奖励A加油卷 此加油卷与充值金额为1：1</label></div>
								<div class="control-group">
									<label class="control-label">加油卷奖励</label>
									<div class="controls">
										<input type="text" name="scroll" id="scroll" value="<?php echo ($scroll); ?>"> % <span>加油卷</span>
									</div>
								</div>
								<div class="well"><label>油卡申领押金：针对所有油卡<br/><br/>1,0代表不收取押金</label></div>
								<div class="control-group">
									<label class="control-label">押金</label>
									<div class="controls">
										<input type="text" name="deposit" id="deposit" value="<?php echo ($deposit); ?>"> % <span>元</span>
									</div>
								</div>
								<div class="well"><label>邮费：只针对普通油卡设置<br/>1,油卡由代理发货时，将会把此邮费退还到代理余额内<br/>2,0代表不收取邮费</label></div>
								<div class="control-group">
									<label class="control-label">邮费</label>
									<div class="controls">
										<input type="text" name="postage" id="postage" value="<?php echo ($postage); ?>"> % <span>元</span>
									</div>
								</div>
								...
								<div class="control-group">
									...
									<div class="controls">
										<input type="text" name="plainprofit" id="plainprofit" value="2.5"> % <span>*</span>
									</div>
								</div>
								<div class="control-group">
									<label class="control-label">VIP会员充值</label>
									<div class="controls">
	...
	<script type="text/javascript">
		$(function(){
			$('.nav-tabs>li').click(function(event) {
				var sid = $('.active').attr('sid');
				if (sid !='s') {$('#profit').hide();}
			});
		})
		$('#saveProfit').click(function(event) {
			var scroll =$('#scroll').val();
			 if(!scroll){$('#scroll').focus();alert('加油卷不能为空！');return false;}
			var	deposit =$('#deposit').val();
			 if(!deposit){$('#deposit').focus();alert('押金不能为空！');return false;}
			var	postage =$('#postage').val();
			 if(!postage){$('#postage').focus();alert('邮费不能为空！');return false;}
			if (confirm('是否重新设置分润标准？')) {
				$.ajax({
					url: "<?php echo U('Setting/ProfitSetting');?>",
					type: 'POST',
					dataType: 'json',
					data: {'scroll': scroll,'postage':postage,'deposit':deposit},
				})
				.done(function(sb) {
					alert('设置成功!');
				})
