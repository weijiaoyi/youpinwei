...
//第三方测试地址
    ...
    public function threeTestNoticePay(){
        $data = file_get_contents('php://input');
        $obj_arr = XML::parse($data);
        if (!$obj_arr) {
            $obj_arr= json_decode($data,TRUE);
        ...
        if( ($cur_sign === $sign && $obj_arr['paymentType'] == 'WxPay' ) || ($obj_arr['paymentType'] == 'HjPay' && $obj_arr['tradeStatus']==1) || ($obj_arr['respcd']=='0000' && $obj_arr['paymentType'] == 'QFPay' )) {
            ...
            $nowTime = time();
            //查询订单记录
            $OrderInfo =  M('test_order')->where(['order_number'=>$OrderSn])->find();
            $userInfo = M('test_user')->where(['three_user_id'=>$OrderInfo['three_user_id']])->find();
            ...
            $Things->startTrans();
            //查看该卡是不是第一次充值
            $is_first = M('test_order')->where(['card_no'=>$OrderInfo['card_no'],'pay_status'=>2])->find();
            if(!$is_first){
                //如果是第一次充值，将该卡状态设置为激活
                M('test_card')->where(['three_card_no'=>$OrderInfo['card_no']])->setField('is_activate',1);
            }


            $order = M('test_order')->where(['order_number'=>$OrderSn])->save($threeOrder);
            $user = M('test_user')->where(['three_user_id'=>$OrderInfo['three_user_id']])->save($User);
            $Tree = M('three')->where(['id'=>$userInfo['three_id']])->find();
            $url = $Tree['url'];
            if($order && $user) {
                $Things->commit();
